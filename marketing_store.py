...
S3_FOLDER = "marketing/"
# S3_MARKETING_INDEX_PATH = "faiss_indexes/marketing_faiss"
...
#             index_path = os.path.join(temp_dir, "marketing_faiss_index")
...
#                 s3_key = f"{S3_MARKETING_INDEX_PATH}{file_name}"
...
def process_marketing_emails():
    """Process emails with improved PDF validation and error handling."""
    try:
        with imaplib.IMAP4_SSL(IMAP_SERVER) as mail:
            mail.login(st.secrets["gmail_Mname"], st.secrets["gmail_Mpwd"])
            logging.info("Email authentication successful")

            mail.select("inbox")
            status, email_ids = mail.search(None, '(SUBJECT "marketing")')
