...
S3_FOLDER = "purchase/"
#S3_PURCHASE_INDEX_PATH = "faiss_indexes/purchase_faiss/"
...
"""def create_faiss_index():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            index_path = os.path.join(temp_dir, "purchase_faiss_index")
            ...
            for file_name in ["index.faiss", "index.pkl"]:
                ...
                s3_key = f"{S3_PURCHASE_INDEX_PATH}{file_name}"
...
def process_purchase_emails():
    """Process emails with improved PDF validation and error handling."""
    try:
        with imaplib.IMAP4_SSL(IMAP_SERVER) as mail:
            mail.login(st.secrets["gmail_username"], st.secrets["gmail_password"])
            logging.info("Email authentication successful")

            mail.select("inbox")
            status, email_ids = mail.search(None, '(SUBJECT "Purchase")')
