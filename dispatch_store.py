...
S3_FOLDER = "dispatch/"
#S3_DISPATCH_INDEX_PATH = "faiss_indexes/dispatch_faiss/"
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for file_name in ["index.faiss", "index.pkl"]:
                ...
                s3_key = f"{S3_DISPATCH_INDEX_PATH}{file_name}"
...
def process_dispatch_emails():
    """Process emails with improved PDF validation and error handling."""
    try:
        with imaplib.IMAP4_SSL(IMAP_SERVER) as mail:
            mail.login(st.secrets["gmail_dname"], st.secrets["gmail_dpass"])
            logging.info("Email authentication successful")

            mail.select("inbox")
            status, email_ids = mail.search(None, '(SUBJECT "Dispatch")')
